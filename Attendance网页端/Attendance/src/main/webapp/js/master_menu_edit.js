//import java.util.ArrayList;
    ...
    function ChangeSelectSubMenu(selectOneMenuId, subSelectId) {
		var zhi=document.getElementById(selectOneMenuId).value;
//		alert("zhi ");
		var url=getRootPathSys() + "ajaxHandlerServlet.ajax?handler=subMenu&id="+encodeURIComponent(escape(zhi));
			...
			_xmlHttpRequestObj.onreadystatechange=function() {
		 		if(_xmlHttpRequestObj.readyState == 4) {
					if(_xmlHttpRequestObj.status == 200) {
					    parseMessageMenuSys("item", subSelectId);
					} else {
	...
	// id : the tag's id
function parseMessageMenuSys(tagName, subSelectOneMenuId) {
  //  	alert(tagName);
